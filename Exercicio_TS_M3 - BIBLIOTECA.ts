...
class Biblioteca {
  ...
  devolverLivro (livro: Livro): void {
    const encontrado = this.acervoLivros.find (l => l === livro);

    if (encontrado) {
      encontrado.Disponivel = true;
      console.log (`O livro "${livro.Titulo}" foi devolvido com sucesso.`);
    } else {
      console.log (`O livro "${livro.Titulo}" não pertence ao acervo da biblioteca ${this.Nome}.`);
    }
  }
}
